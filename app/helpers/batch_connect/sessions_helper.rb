module BatchConnect::SessionsHelper
  ...
  def created(session)
    content_tag(:p) do
      concat content_tag(:strong, t('dashboard.bc.sessions.stats.created_at'))
      concat " "
  ...
  def host(session)
    content_tag(:p) do
      concat content_tag(:strong, t('dashboard.bc.sessions.stats.host'))
      concat " "
      concat session.connect.host || t('dashboard.bc.sessions.stats.undetermined_host')
    end if session.running?
  end

  def id(session)
    content_tag(:p) do
      concat content_tag(:strong, t('dashboard.bc.sessions.stats.session_id'))
      concat " "
      concat(
        link_to(
          session.id,
  ...
  def delete(session)
    link_to(
      icon("fas", "trash-alt", t('dashboard.bc.sessions.delete.title'), class: "fa-fw"),
      session,
      method: :delete,
      class: "btn btn-danger pull-right btn-delete",
      title: t('dashboard.bc.sessions.delete.hover'),
      data: { confirm: t('dashboard.bc.sessions.delete.confirm'), toggle: "tooltip", placement: "bottom"}
    )
  end
  ...
  def session_save_errors(errors)
    capture do
      errors.keys.each do |key|
        case key
        when :submit
          concat content_tag(:h4, t('dashboard.bc.sessions.errors.submission'))
          concat content_tag(:pre, errors[key].first)
        when :stage
          concat content_tag(:h4, t('dashboard.bc.sessions.errors.staging'))
          concat content_tag(:pre, errors[key].first)
        else
