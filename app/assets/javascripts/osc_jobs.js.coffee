...
@request_job_data = (id) ->
  if id?
    ...
    $.ajax
      type: 'GET'
      url: app_root() + Routes.osc_job_path(id)
...
@update_copy_button = (id) ->
  if id?
    $("#copy_button").attr("href", app_root() + Routes.copy_osc_job_path(id))
...
@update_submit_button = (id, status_char) ->
  if id? && !status_char?
      $("#submit_button").attr("href", app_root() + Routes.submit_osc_job_path(id))
...
@update_template_button = (path, host) ->
  if path?
    $("#template_button").attr("href", app_root() + Routes.new_template_path({ path: path, host: host }))
...
@update_destroy_button = (id) ->
  if id?
    $("#destroy_button").attr("href", app_root() + Routes.osc_job_path(id))
