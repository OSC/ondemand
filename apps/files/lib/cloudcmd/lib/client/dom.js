    ...
    var DOMFunc                     = function() {},
        ...
        ImagesProto                 = function() {
            var Images,
                ImageElementProto   = function() {
                    ...
                    /* The function creates a load image */
                    ...
                    /* The function creates the error image */
                    this.error      = function() {
                        var element     = getElement(),
                            classList   = element.classList;
        ...
        CmdProto                    = function() {
            ...
            function promptNew(typeName, type) {
                ...
                Dialog.prompt(TITLE, msg, "", {cancel: false}).then(function(name) {
                    path        = dir + name;
                    ...
                    if (name){
                      //FIXME: this is not fool proof but easiest to implement
                      //foolproof would be to have the API expect a POST to
                      //create a file and a PUT to update a file and throw and
                      //error if you are PUT-ing to a file that doesn't exist
                      //or POST-ing to a file that already exists
                      if(DOM.getCurrentByName(name)){
                        Dialog.alert(TITLE, 'This ' + typeName + ' already exists. Please provide a unique name.');
                      }else{
            ...
            this.getCurrentSize          = function(currentFile) {
                ...
                /* If a folder - return dir instead of the word size */
                size    = size
                    .textContent
                    .replace(/^<|>$/g, '');
                        ...
                        if (from !== to) {
                            ...
                            RESTful.mv(files, function(error) {
                                ...
                                if (!error) {
                                    ...
                                    // OSC_CUSTOM_CODE Always refresh
                                    // Fixes https://github.com/OSC/ood-fileexplorer/issues/132
                                    // if (isExist)
                                    CloudCmd.refresh();
