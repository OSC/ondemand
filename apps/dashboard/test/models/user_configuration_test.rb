...
class UserConfigurationTest < ActiveSupport::TestCase
  ...
  test 'properties expected defaults' do
    defaults = {
      ...
      announcement_path:                 [Pathname.new('/etc/ood/config/announcement.md'),
                                          Pathname.new('/etc/ood/config/announcement.yml'), Pathname.new('/etc/ood/config/announcements.d')],
...
user_settings_profile: {} } })
    ...
    UserConfiguration::USER_PROPERTIES.each do |property|
      ...
      property.environment_names.each do |env_var|
        ...
        with_modified_env({ env_var => value_in_env }) do
          assert_equal property_value, target.send(property.name),
                       "#{property.name} should have responded to ENV['#{env_var}']=#{ENV[env_var]}"
        end
      end
    end
  end
  ...
  test 'USER_PROPERTIES should respond to ConfigurationSingleton.config' do
    ...
    target = UserConfiguration.new
    UserConfiguration::USER_PROPERTIES.each do |property|
      assert_equal value_in_config, target.send(property.name),
                   "#{property.name} should have been #{value_in_config} through ConfigurationSingleton."
    end
  end
  ...
  test 'USER_PROPERTIES should respond to profile configuration in ConfigurationSingleton.config' do
    ...
    UserConfiguration::USER_PROPERTIES.each do |property|
      assert_equal value_in_profile, target.send(property.name),
                   "#{property.name} should have been #{value_in_profile} through profile strings_profile in ConfigurationSingleton"
    end
  end

  test 'env variables have precedence in USER_PROPERTIES when read_from_environment is enabled' do
    ENVIRONMENT_PROPERTIES.transform_values do |property_info|
      property_info[1].to_s
    end
    ...
    UserConfiguration::USER_PROPERTIES.each do |property|
      ...
      property.environment_names.each do |env_var|
        ...
        with_modified_env({ env_var => expected_value.to_s }) do
          assert_equal expected_value, target.send(property.name),
                       "Value for #{property.name} as environment: #{env_var} should have been #{expected_value}"
        end
      end
    end
  end
  ...
  test 'ensure all properties that can have an environment value are verified' do
    ...
    UserConfiguration::USER_PROPERTIES.each do |property|
      ...
      property.environment_names.each do |env_var|
        assert_equal true, ENVIRONMENT_PROPERTIES.key?(env_var),
                     "ensure property: #{property.name} with environment key: #{env_var} is added to ENVIRONMENT_PROPERTIES"
      end
    end
    ...
    ENVIRONMENT_PROPERTIES.each do |env_var, property_info|
      ...
      with_modified_env({ env_var => expected_value.to_s }) do
        assert_equal expected_value, target.send(property),
                     "Value for #{property} as environment: #{env_var} should have been #{expected_value}"
      end
    end
  end
