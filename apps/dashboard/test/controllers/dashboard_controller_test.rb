...
class DashboardControllerTest < ActionController::TestCase
  ...
  test "should not create app menus if NavConfig.categories is empty and allowlist is enabled" do
  ...
  test "should exclude gateway apps if NavConfig.categories is set to default and allowlist is enabled" do
    SysRouter.stubs(:base_path).returns(Rails.root.join("test/fixtures/sys_with_gateway_apps"))
    OodAppkit.stubs(:clusters).returns(OodCore::Clusters.load_file("test/fixtures/config/clusters.d"))
    NavConfig.stubs(:categories_allowlist?).returns(true)
  ...
  test "uses NavConfig.categories as sort order if allowlist is false" do
    SysRouter.stubs(:base_path).returns(Rails.root.join("test/fixtures/sys_with_gateway_apps"))
    OodAppkit.stubs(:clusters).returns(OodCore::Clusters.load_file("test/fixtures/config/clusters.d"))
  ...
  test "verify default values for NavConfig" do
    refute NavConfig.categories_allowlist?
    assert NavConfig
  end

  test "display all app menus in alphabetical order if NavConfig.allowlist false & NavConfig.categories nil or []" do
    SysRouter.stubs(:base_path).returns(Rails.root.join("test/fixtures/sys_with_gateway_apps"))
    OodAppkit.stubs(:clusters).returns(OodCore::Clusters.load_file("test/fixtures/config/clusters.d"))
    NavConfig.stubs(:categories_allowlist?).returns(false)
    NavConfig.stubs(:categories).returns([])
