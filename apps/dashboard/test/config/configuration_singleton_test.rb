...
class ConfigurationSingletonTest < ActiveSupport::TestCase
  ...
  test "logs read and parse errors" do
    with_modified_env(config_fixtures) do
      bad_erb_rex = %r{bad_erb\.yml\.erb.*undefined (?:local variable or method|method).*wont_find_this_functon}i
      bad_yml_rex = %r{not_good_yml\.yml.*did not find expected '-'}i
