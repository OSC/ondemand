    ...
    def render_connection(session)
      if session.running?
        if session.view
          ...
          views = []
          views << { title: 'noVNC Connection', partial: 'novnc',
locals: { connect: session.connect, app_title: session.title } }
          if ENV['ENABLE_NATIVE_VNC']
            views << { title: 'Native Instructions', partial: 'native_vnc',
locals: { connect: session.connect } }
    ...
    def session_time(session)
      ...
      if session.starting? || session.running?
        if time_limit.to_i.positive? && time_used
          [t('dashboard.batch_connect_sessions_stats_time_remaining'),
           distance_of_time_in_words(time_limit - time_used, 0, false, :only          => [:minutes, :hours],
                                                                       :accumulate_on => :hours)]
        elsif time_used
          [t('dashboard.batch_connect_sessions_stats_time_used'),
           distance_of_time_in_words(time_used, 0, false, :only => [:minutes, :hours], :accumulate_on => :hours)]
        end
      elsif time_limit.to_i.positive?
        [t('dashboard.batch_connect_sessions_stats_time_requested'),
         distance_of_time_in_words(time_limit, 0, false, :only => [:minutes, :hours], :accumulate_on => :hours)]
      end
    end
    ...
    def relaunch(session)
      ...
      params = batch_connect_app.attributes.map do |attribute|
        ["batch_connect_session_context[#{attribute.id}]", user_context.fetch(attribute.id, nil)]
      end.to_h.compact
      ...
      button_to(
        ...
        method:       :post,
        class:        ['btn', 'px-1', 'py-0', 'btn-outline-dark', 'relaunch', 'full-page-spinner'],
        form_class:   ['d-inline', 'relaunch'],
        title:        title,
        'aria-label': title,
        data:         { toggle: 'tooltip', placement: 'left' },
        params:       params
      ) do
        fa_icon('sync', classes: nil, title: nil).to_s.html_safe
      end
    end
    ...
    def edit(session)
      ...
      button_to(
        ...
        method:       :get,
        class:        ['btn', 'px-1', 'py-0', 'btn-outline-dark', 'full-page-spinner'],
        form_class:   ['d-inline', 'edit-session'],
        title:        title,
        'aria-label': title,
        data:         { toggle: 'tooltip', placement: 'left' },
        params:       { session_id: session.id }
      ) do
        fa_icon('pen', classes: nil, title: nil).to_s.html_safe
      end
    end
    ...
    def delete(session)
      ...
      button_to(
        ...
        title:        title,
        'aria-label': title,
        data:         { confirm: t('dashboard.batch_connect_sessions_delete_confirm'), toggle: 'tooltip',
placement: 'bottom' }
      ) do
        "#{fa_icon('times-circle',
                   classes: nil)} <span aria-hidden='true'>#{t('dashboard.batch_connect_sessions_delete_title')}</span>".html_safe
      end
    end
    ...
    def cancel(session)
      ...
      button_to(
        ...
        title:        title,
        'aria-label': title,
        data:         { confirm: t('dashboard.batch_connect_sessions_cancel_confirm'), toggle: 'tooltip',
placement: 'bottom' }
      ) do
        "#{fa_icon('times-circle',
                   classes: nil)} <span aria-hidden='true'>#{t('dashboard.batch_connect_sessions_cancel_title')}</span>".html_safe
      end
    end
    ...
    def novnc_link(connect, view_only: false)
      ...
      asset_path(
        "noVNC-#{version}/vnc.html?autoconnect=true&password=#{password}&path=rnode/#{connect.host}/#{connect.websocket}/websockify&resize=#{resize}", skip_pipeline: true
      )
    end
            ...
            content_tag(:ul, class: 'nav nav-tabs') do
              tabs.map { |t| t[:title] }.map.with_index do |title, idx|
                content_tag(:li, class: "nav-item #{'active' if idx.zero?}") do
                  link_to title, "#c_#{id}_#{idx}", data: { 'bs-toggle': 'tab' },
  aria: { selected: (true if idx.zero?) }, class: "nav-link #{'active' if idx.zero?}"
                end
              end.join("\n").html_safe
            end
          )
          ...
          concat(
            content_tag(:div, class: 'tab-content') do
              tabs.map.with_index do |tab, idx|
                content_tag(:div, id: "c_#{id}_#{idx}", class: "tab-pane ood-appkit markdown #{'active' if idx.zero?}",
  role: 'tabpanel') do
                  render partial: "batch_connect/sessions/connections/#{tab[:partial]}", locals: tab[:locals]
                end
              end.join("\n").html_safe
