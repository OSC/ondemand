...
jQuery(function (){
    const SPINNER_HTML = `<div class="app-launcher-spinner">
      <div class="spinner-border" role="status"></div>
    </div>`;
    const launcherSelector =
      "[data-toggle='launcher-button'], [data-bs-toggle='launcher-button']";
    const launcherClickSelector = `${launcherSelector} .launcher-click`;
    const pinnedAppsSearch = document.getElementById('pinned-apps-search');
    const pinnedAppItems = Array.from(
      document.querySelectorAll('.pinned-app-checkbox-item')
    );
    ...
    function showSpinner(event) {
        ...
        $(launcherSelector).removeClass("app-launcher-hover");
    }

    function hideSpinner() {
        $(".app-launcher-container").removeClass("app-launcher-disabled");
        $(launcherSelector).addClass("app-launcher-hover");
        $("div.app-launcher-spinner").remove();
    }

    $(launcherClickSelector).each((index, element) => {
    ...
    if (pinnedAppsSearch) {
      pinnedAppsSearch.addEventListener('input', () => {
        ...
        pinnedAppItems.forEach((item) => {
          ...
          const tokenMatch = token.includes(query);
          const matches = query.length === 0 || titleMatch || tokenMatch;
    ...
    if (clearAllBtn) {
      clearAllBtn.addEventListener('click', () => {
        setCheckedForVisible(false);
      });
    }

    /*
    Back button fix. Because of browser page caching, spinner can stay visible.
    This event triggers on page show and removes the spinner if present.
     */
