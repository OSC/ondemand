...
module FileSystem
    class Entry
        attr_reader :basename, :fs_type, :group, :modified, :owner, :path, :permissions, :size
        ...
        def self.from_pathname(path)
            ...
            new(
                ...
                size: Filesize.new(stat.size).pretty,
            )
        end

        def initialize(basename:, group:, modified:, owner:, path:, permissions:, size:, fs_type:)
            @basename = basename
            @fs_type = fs_type
            @group = group
            @modified = modified
