...
class HpcModule
          ...
          begin
            JSON.parse(File.read(file)).map do |name, spider_output|
              spider_output.map do |_, mod|
                desc =
                  mod['whatis']&.find { |line| line.downcase.start_with?('short description') }&.split(':', 2)&.last&.strip ||
                  mod['help']&.strip ||
                  "No description available"
                HpcModule.new(name, version: mod['Version'], description: desc, hidden: mod['hidden'])
              end
  ...
  attr_reader :name, :version, :description, :hidden
  attr_accessor :cluster
  alias hidden? hidden

  def initialize(name, version: nil, description: nil, hidden: false)
    @name = name
    @version = version.to_s if version
    @description = description
    @hidden = hidden
    Rails.logger.debug("HpcModule: #{name} version: #{version} description: #{description}")
  end
