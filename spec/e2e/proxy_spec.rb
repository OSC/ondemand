...
describe 'Node and Rnode proxies' do
  ...
  it 'node correctly returns 404 when no redirect is given on init' do
  ...
  it 'node correctly returns 404 when redirect to "github.com" given on init' do
    browser.goto "#{ctr_base_url}/nginx/init?redir=github.com"
    expect(response.status).to have_http_status(404)
  end

  it 'node correctly returns 404 when redirect to "https://github.com" given on init' do
    browser.goto "#{ctr_base_url}/nginx/init?redir=https://github.com"
    expect(response.status).to have_http_status(404)
  end

  it 'node correctly returns 404 when redirect to "/node/localhost/5001" is given on init' do
  ...
  it 'node correctly returns 404 when no redirect is given on stop' do
  ...
  it 'node correctly returns 404 when redirect to "github.com" given on stop' do
    browser.goto "#{ctr_base_url}/nginx/stop?redir=github.com"
    expect(response.status).to have_http_status(404)
  end

  it 'node correctly returns 404 when redirect to "https://github.com" given on stop' do
    browser.goto "#{ctr_base_url}/nginx/stop?redir=https://github.com"
    expect(response.status).to have_http_status(404)
  end

  it 'node correctly returns 404 when redirect to "/node/localhost/5001" is given on stop' do
  ...
  it 'node correctly redirects to /pun/sys/dashboard with 307 when no redirect is given on noop' do
    browser.goto "#{ctr_base_url}/nginx/noop"
    expect(browser.url).to eq("#{ctr_base_url}/pun/sys/dashboard")
    expect(response.status).to have_http_status(307)
  end 

  it 'node correctly returns 404 when redirect to "github.com" given on noop' do
    browser.goto "#{ctr_base_url}/nginx/noop?redir=github.com"
    expect(response.status).to have_http_status(404)
  end

  it 'node correctly returns 404 when redirect to "https://github.com" given on noop' do
    browser.goto "#{ctr_base_url}/nginx/noop?redir=https://github.com"
    expect(response.status).to have_http_status(404)
  end

  it 'node correctly returns 404 when redirect to "/node/localhost/5001" is given on noop' do
